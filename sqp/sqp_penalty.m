function [x,success] = sqp_penalty(x0, Q, q, f, A_ineq, b_ineq, A_eq, b_eq, g, h, user_cfg)
...
cfg.nU = 0;
cfg.T = 0;
cfg.improve_ratio_threshold = 2.25;
...
penalty_iter = 1;
max_penalty_iter = cfg.max_penalty_iter;
while true    
    % YOUR CODE HERE    
    ...
    if (penalty_iter >= max_penalty_iter || sum(max(g(x),0))+sum(abs(h(x))) < cfg.cnt_tolerance)
        break;
    end  
...
if any(A_ineq*x0 > b_ineq) || any(A_eq*x0 ~= b_eq)
    ...
    subject to
        ...
        A_eq*x == b_eq;
    cvx_end
    fprintf('Done finding feasible init point\n');
...
function [x, trust_box_size, success] = minimize_merit_function(x, Q, q, ...
    ...
    while  true
        ...
        % constraints f and g.
        fprintf('  sqp iter: %i\n', sqp_iter);
%         fprintf('  computing f gradients\n');            
        if cfg.f_use_numerical
            fval = f(x);
            %because we don't have objective function f
...
%         fprintf('  computing h gradients\n');            
        ...
        while true 
            ...
            cvx_begin quiet
                ...
                minimize( f(xp) + fquadlin(x) + (q'+Q*x)'*(xp-x)+ penalty_coeff*hinge(g(x)+gjac*(xp-x)) + penalty_coeff*abssum(h(x)+hjac*(xp-x)) );
                subject to 
            ...
            % This function call provides debug/progress information about SCP
            sqp_view_progress(xp,cfg,sqp_iter)
            ...
            if approx_merit_improve < -1e-5
                ...
                success = false;
                return;
            elseif (approx_merit_improve < cfg.min_approx_improve || sqp_iter > cfg.max_iter)
