function [gval, jach] = g_nadirConstraintJach(xu_trajectory,cfg,sats)

    gval = g_nadirConstraint(xu_trajectory,cfg,sats);    
...
function jach = computeJacobian(xu_trajectory,cfg,sats)
    ...
    jach = zeros(2*T,nXUT); %G IS A TWO_VALUED FUNCTION
    eps = 1e-3;
    delta = zeros(nX+nU,1);
    single_cfg.nX = nX; single_cfg.nU = nU; single_cfg.T = 1;
    g = @(x) g_nadirConstraint(x,single_cfg,sats);
        ...
        if (i <= nX*T)
            ...
            if ~(ind == 1 || ind == 3 || ind == 4 || ind == 5 || ind == 6 ...
               || ind == 1+9 || ind == 3+9 || ind == 4+9 || ind == 5+9 || ind == 6+9)
                ...
                jach(indX:indX+1,i) = (yhi - ylo)/eps;
                delta(ind) = 0;
            end
        else
%             %variations on u_t-1
